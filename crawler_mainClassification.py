...
class Statistics:
    ...
    def __init__(self):
        ...
        # txtD: year
        # txtG: main classification number
        # txtQ: province name
    ...
    def getPage(self, year, place, name):
        ...
        queryData = re.sub(re.compile('txtG=[^&]*&'), 'txtG='+ name +'&',queryData)
        queryData = re.sub(re.compile('txtD=[^&]*&'), 'txtD='+ year +'&',queryData)
        queryData = re.sub(re.compile('strWhere=[^&]*&'), 'strWhere=pd=('+year+')+and+pic=('+name+')+and+co=('+place+')'+'&',queryData)
    ...
    def getResult(self):
        ...
        f = open('result_mainClassification.txt', 'w')
        f.truncate() 
